...
- Python 核心与接口：`python3 -m unittest discover -s tests -v`，42 项通过。
...
- 真实 Linux 部署：完成 Skill 安装、项目台账配置、GPU 探测、已有 Python 环境调用、实验状态文件读取与原生 tmux 查询。
...
- 安装包验证：在隔离虚拟环境安装 wheel 后，`computemate` 可读取项目、服务器记录及完整的 55 项能力目录；VS Code Extension Host 多项目验证通过。
- 首次使用复验：干净安装、独立 Skill、README 示例台账、导入导出和构建后重新安装通过；5 项前端结果展示回归通过，真实浏览器验证新增、搜索、失败反馈、防重复提交、令牌恢复及 420px 窄窗口。真实 VS Code 宿主新增第三个未初始化项目，确认初始化后工作台可正常读取空台账。问题和修复见 [首次使用检查](docs/usability-review.md)。
